...
from threading import Thread, Lock#멀티 스레드를 사용하므로 Lock 이용
...
class ARPTable:
    ...
    def get_ARP_table(self, interface:str, ips:str) -> int:
        ...
        conf.verb = 3
        addr, subnet = ips.split('/')
        bin_addr = ''.join(format(int(i),'08b') for i in addr.split('.'))
        baseip = bin_addr[:int(subnet)]
        ans = list()
        lock = Lock()

        def send_p_thread(packet,interface,ans,f):
            r = srp(packet,iface = interface, timeout = 5,verbose = 3, filter = f)
            with lock:
                ans.append(r)
        ...
        for i in range(2**(32-int(subnet))):
            destip = baseip+(format(int(i),str(32-int(subnet))+'b'))
            dest_addr = ''.join((str(int(destip[8*j:8*j+8],2))+'.') for j in range(4))
            dest_addr = dest_addr[:-1]
            packet = Ether(dst = "ff:ff:ff:ff:ff:ff")/ARP(pdst = dest_addr)
            f = "host "+dest_addr + " and arp and arp[6:2] = 2"
            trd = Thread(target = send_p_thread, args = (packet,interface,ans,f))
            trds.append(trd)
            trd.start()
        ...
        for snd, rcv in ans:
            ...
            for p in snd:
                if p[1][ARP].op == 2:
                    ip_addr = p[1][ARP].psrc
                    mac_addr = p[1][ARP].hwsrc
